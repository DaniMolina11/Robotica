...
class MazeSolver(Node):
    ...
    def control_loop(self):
        ...
        # --- DETECCIÓN CALLEJÓN ---
        # Si está completamente bloqueado -> Girar 180º en el sitio
        if self.estado in ('avanzar', 'pasillo') and self.d_front <= 0.22 and self.d_left < 0.30 and self.d_right < 0.30:
        ...
        if self.estado == 'girar_180':
            twist.linear.x = 0.0 # Giro estático puro
            twist.angular.z = VEL_GIRO
            evento = 'girando_180_estatico'
            
        elif self.estado == 'girar_izq':
            # SEGURIDAD: Si está muy cerca de pared lateral, stop lineal
            twist.linear.x = VEL_AVANCE_GIRO if (self.d_front > 0.20 and self.d_left > 0.20) else 0.0
            twist.angular.z = VEL_GIRO
            evento = 'girar_izq'
            
        elif self.estado == 'girar_der':
            # SEGURIDAD: Si está muy cerca de pared lateral, stop lineal
            twist.linear.x = VEL_AVANCE_GIRO if (self.d_front > 0.20 and self.d_right > 0.20) else 0.0
            twist.angular.z = -VEL_GIRO
            evento = 'girar_der'
