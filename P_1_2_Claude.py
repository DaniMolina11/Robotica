...
TIEMPO_GIRO_180        = 3.5    # Tiempo que el robot tarda en girar 180º sobre su eje
...
class MazeSolver(Node):
    ...
    def _cambiar_estado(self, nuevo, motivo=''):
        if nuevo != self.estado:
            self._log_raw(f'*** ESTADO {self.estado} -> {nuevo} [{motivo}]')
            self.estado = nuevo

    def _decidir_lado_giro(self):
        en_pasillo = (self.d_right < DIST_PASILLO and self.d_left < DIST_PASILLO)
        return 'izq' if (en_pasillo and self.d_diag_izq >= self.d_diag_der) or (not en_pasillo and self.d_left >= self.d_right) else 'der'
    ...
    def control_loop(self):
        ...
        # Si frente, izq y der < 0.25 -> Girar 180 sobre el eje
        if self.estado == 'avanzar' and self.d_front < 0.25 and self.d_left < 0.25 and self.d_right < 0.25:
            self._cambiar_estado('girar_180', 'callejon detectado')
            self.tiempo_inicio_giro = ahora
            return

        # --- MÁQUINA DE ESTADOS ---
        if self.estado == 'pasillo':
            if (self.d_right < DIST_PASILLO and self.d_left < DIST_PASILLO):
                if self.d_front < DIST_GIRO_PASILLO:
                    self._iniciar_giro(ahora)
            else:
                self._cambiar_estado('avanzar', 'salida pasillo')
