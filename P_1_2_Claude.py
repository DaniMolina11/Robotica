...
TIEMPO_GIRO_180        = 3.5    # Tiempo para girar 180º (ajustar si gira de más o de menos)
...
class MazeSolver(Node):
    ...
    def control_loop(self):
        ...
        if self.estado in ('avanzar', 'pasillo'):
            # Detecta callejón: frente, izquierda y derecha bloqueados
            if d_f <= 0.24 and d_l < 0.30 and d_r < 0.30:
                self._cambiar_estado('girar_180', 'callejon detectado: 180 grados')
                self.tiempo_inicio_giro = ahora
                self.giro_comprometido = False
                self.reset_filtros()
                return
        ...
        if self.estado == 'pasillo':
            ...
            # GIRO SOBRE EL EJE: 0.0 lineal, angular constante
            twist.linear.x = 0.0
            twist.angular.z = VEL_GIRO
            evento = 'girando 180 estático'
